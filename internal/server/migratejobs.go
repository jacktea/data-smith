...
func (s *Server) handleRollbackSubmit(w http.ResponseWriter, r *http.Request) {
	...
	job := s.jobs.Submit(JobRollback, map[string]any{
		...
		return s.runRollback(ctx, job, req.LibraryID, stored.ID)
	})
	writeJSON(w, http.StatusOK, map[string]any{"job": job.View()})
}

func (s *Server) runRollback(ctx context.Context, job *Job, libraryID, connectionID string) error {
	libraryDir, err := s.libraryDir(libraryID)
	if err != nil {
		return err
	}
	...
	job.Summary(map[string]any{"version": version, "rolled": true})
