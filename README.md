...
MySQL/PostgreSQL 的 DML 生成共用同一行定位策略：主键优先，其次普通、完整、
全列非空的唯一索引；无可靠身份时 DELETE 用排序后的全行字段定位、UPDATE
不生成。共享层只选择列并携带列类型和值，标识符引用、值字面量和最终 SQL
仍由各方言独立生成。
...
通过持久化发布日志作为一组发布；进程中断或主机重启后，下一次运行会恢复完整
旧组或确认完整新组，不暴露混合产物。`--data-diff-mode shadow` 强制影子（仅 PostgreSQL source），
`direct` 强制禁用。Web 控制台的「完全比对」页在高级参数中提供同一开关
（数据比对模式：自动/强制影子事务/直接比对，默认自动），任务参数与日志
区分请求模式与生效模式：`requested` 保留 `auto|shadow|direct`，结构比对后
`effective` 只记录 `shadow|direct`；任务 API 摘要、落盘 `summary.json` 与日志
使用同一组值。引擎语义（含 MySQL 拒绝强制影子）与 CLI 完全一致。
...
Web 控制台通过 `./datasmith web` 启动，默认仅监听 `127.0.0.1:8080`；显式配置
非 loopback `--addr` 会提示当前没有远程认证，生产远程访问必须另行配置可信
网络边界与访问控制。SQL 控制台执行写入脚本时必须显式选择当前连接的
`source`/`target` 角色；Web 与 CLI 共用同一 EXECUTE-ON 硬校验，目标不一致、
非法/空标记或缺少目标角色都会在数据库执行前拒绝；切换物理连接会清空此前
选择的角色，必须针对新连接重新确认。
...
> Web 脚本库以 up（含省略方向、会被解析为 up 的旧式文件名）作为版本的可执行
> 入口和元数据生命周期依据：删除 up 时即使 down 仍保留也清理版本登记；只删
> down 且 up 仍在则保留；up/down 均不存在时重复清理保持幂等。控制台启动时会
> 执行同一规则的 store 一致性自检。目录扫描或元数据持久化失败不会伪装成普通
> 成功：删除接口明确返回“脚本已删除、元数据清理失败”的非 2xx 部分成功结果，
> 启动自检失败则阻止启动并返回错误。
> 「迁移管理 → 脚本库管理」提供两种复制能力：**复制脚本库**把当前库的全部
> 脚本（含 down/JSON）与版本登记元数据（含预期连接）克隆为一个全新脚本库；
> 勾选脚本后点**复制到其他库**可跨库批量复制脚本，目标库存在同名脚本或同
> 版本不同标题的脚本时会整批拒绝并列出冲突清单，不会产生部分复制结果。
