...
初始评审为只读评审；2026-09-20 已在起始提交 `0742d0e` 上完成阶段 1（O1、O2、O3、S2）整改，状态见各发现及第 8 节。

## 2. 结论

本轮增强补充了 PostgreSQL 普通函数、存储过程、序列、CHECK、视图注释、主键背书索引、整库规则展开与 PostgreSQL 影子事务数据比对，方向正确，现有自动化门禁全部通过。

但当前版本仍不适合直接用于无人值守生产迁移。主要问题已经从“对象完全缺失”转为“对象建模不闭环、组合场景不正确”：序列 ownership 未参与 diff/DDL，对象依赖仍依靠固定阶段排序，单侧表的数据迁移语义不完整，视图定义与注释同时变化时漏更新，影子事务和四产物发布也没有完整失败保证。

函数和序列已进入当前功能范围，但属于 PostgreSQL 首版部分支持，而不是完整数据库对象迁移能力。

## 3. Standards 发现

### S1【P1】例程、序列和表使用固定阶段排序，缺少对象依赖图
...
### S5【P2】序列 ownership 建模后没有进入相等判断和 SQL
...
- PostgreSQL：普通函数（`prokind=f`）、存储过程（`prokind=p`）、独立序列与 SERIAL 从属序列的提取和基础 SQL 生成。

### 部分支持

- PostgreSQL sequence ownership：能提取，不能正确比较和重建。
- 函数/过程：能按身份签名 diff 定义，但缺跨对象依赖排序；返回类型等不兼容变更缺少安全规划。
...
初始评审的门禁在 `0742d0e` 上通过。阶段 1 整改工作树在同日起点上重新执行以下完整门禁并全部通过：
...
全部门禁通过不等于上述组合缺陷不存在；当前测试主要缺少跨对象依赖、同时变化、单侧表和失败注入场景。
