...
- 最终复审覆盖率：overall 74.8%（6206/8296）、db 78.9%（674/854）、diff 77.8%（1859/2390）、sql 76.3%（1236/1620）、migrate 75.9%（480/632）、exec 89.1%（376/422）。
- 最终复审补充：SQL 控制台切换物理连接会清空角色；diff-full 使用持久化发布日志从中断状态恢复完整旧组或新组；同一脚本库的文件与版本元数据变更使用 per-library 串行化。
...
- `diff-full` 先在同目录 staging 中生成四产物，再通过持久化发布日志作为一组发布；数据阶段、写盘、发布失败及模拟进程中断均验证只恢复完整旧组或保留完整新组，且无暂存泄漏。
